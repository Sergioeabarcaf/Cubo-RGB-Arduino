...
void setup() {
  ...
  while(true){
    // draw some text!
    matrix.setCursor(i+20,1);    // start at top left, with one pixel of spacing
    matrix.setTextSize(1);     // size 1 == 8 pixels high
    matrix.setTextWrap(false); // Don't wrap at end of line - will do ourselves
    
    matrix.setTextColor(matrix.Color333(7,0,0));
    //soporta palabras de hasta 7 caracteres
    matrix.println("Feliz");
    matrix.setCursor(i+10,11);
    matrix.setTextColor(matrix.Color333(0,7,0));
    matrix.println("Navidad");
    //Palabras mayores a 7 caracteres se deben deletrar
    matrix.setCursor(i,23);
